...
export const useTheme = () => {
  ...
  const normalizedPreference = computed<ThemePreference>(() =>
    isThemePreference(preference.value) ? preference.value : defaultThemePreference,
  )
  ...
  const resolvedTheme = computed<ThemeVariant>(() =>
    resolveThemeVariant(normalizedThemeName.value, normalizedPreference.value, systemPrefersDark.value),
  )
